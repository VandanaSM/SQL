...
/*ORDER BY*/
SELECT * FROM AirportDetails order by id desc;
/*LIKE*/
SELECT * FROM AirportDetails WHERE nameOfFlight like '%A%';

/*GROUP BY*/
SELECT nameOfFlight,count(*) from AirportDetails Group by nameOfFlight;
SELECT * FROM AirportDetails ORDER BY nameOfFlight;
SELECT DISTINCT(nameOfFlight) FROM AirportDetails;
/*having---SELECT DISTINCT FROM WHERE GROUP BY HAVING ORDER BY*/
SELECT nameOfFlight ,sum(bookingAmount) as sum from AirportDetails Group by nameOfFlight HAVING sum>12500;
SELECT instr('xworkzodc','R') as position;
SELECT substr('xworkzodc',4,5)AS substring;

create table student(id int not null,name varChar(20) unique,age int check(age>=18),
location varChar(20) default 'Bangalore');
Insert into AirportDetails values(1,'Basu',27,'Gokak');
Insert into AirportDetails values('Punith',25,'Bng');
Insert into AirportDetails values(null,'Sri',20,'Bangalore');

Insert into AirportDetails (id,name,age)values(4,'nikhil',24);  /* to give data to the column names*/
